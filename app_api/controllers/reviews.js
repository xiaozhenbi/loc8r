            ...
            .exec(
                function (err, location) {
                    if (err) {
                        ...
                        // Successful find operation will call
                        // new function to add review, passing
                        // request, response, and location objects
                        doAddReview(req, res, location);
                    }
                }
            );
    } else {
        sendJsonReponse(res, 404, {"message": "locationid required"});
    }
};
...
module.exports.reviewsReadOne = function (req, res) {
    ...
    if (req.params && req.params.locationid && req.params.reviewid) {
        Loc
            ...
            // stating that we want to get name of location and
            // its reviews
            .select('name reviews')
            .exec(
                function (err, location) {
                    ...
                    if (location.reviews && location.reviews.length > 0) {
                        // Use Mongoose subdocument.id method as
                        // a helper for searching for matching ID
                        ...
                        if (!review) {
                            ...
                            response = {
                                // If review is found build response object returning
